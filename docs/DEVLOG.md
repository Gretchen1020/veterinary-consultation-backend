...
## Loose-End Cleanup — History, Support Enquiries, Mark Paid, is_read Removal

### Updates to earlier days
- **`pending.php` (BE-10, Day 6) — added the mentor-confirmed `?status=` filter.** Was hardcoded to `approval_status = 'pending'` only, despite the contract calling for a filter. Whitelisted against `pending`/`approved`/`rejected`; defaults to `pending` when omitted, so existing callers are unaffected.
- **`docs/schema.sql` resynced three times this session:** (1) caught up on the Day 13 `doctor_earnings.status` migration that was never reflected in docs, (2) added `doctor_earnings.paid_at`/`paid_by` for BE-18b, (3) `chat_messages.is_read` dropped entirely (see Design decisions).

### Endpoints
- `GET /api/chat/history.php` (BE-17) — new. List mode (no `session_id`) returns paginated past-session summaries scoped to the caller; transcript mode (`?session_id=`) returns the message log for one session via `getAuthorizedSession(..., requireActive: false)` — the only path that can read an *ended* session's messages, since `messages.php`'s GET requires an active one.
- `POST/GET /api/support/enquiries.php` (BE-19) — new. POST is public (optionally links `user_id` if a session exists), GET is admin-only paginated list. Includes T-16 rate limiting (same-email-within-60s) and length caps.
- `POST /api/support/update-status.php` (BE-19b, not in original contract) — new. Admin-only, marks an enquiry resolved, one-directional — mirrors BE-11's "decision is final" precedent.
- `POST /api/admin/earnings-mark-paid.php` (BE-18b, not in original contract) — new. Admin-only, marks a `doctor_earnings` row paid. Update in place chosen over an append-only payout-ledger alternative, justified by the spec's own "ledger/settlement view" + "Insert/list/summary" wording. `paid_at`/`paid_by` added via migration so the in-place update doesn't lose the audit trail an append-only design would've given for free.

### Design decisions
- BE-17's list-vs-transcript split via presence of `?session_id=` — not spelled out in the contract, flagged for mentor confirmation.
- BE-18b deliberately breaks the append-only pattern used elsewhere (update-in-place). Single-row only — a bulk "settle all unpaid for doctor X" action was not built, to avoid unrequested scope.
- BE-19's rate limit (same-email/60s, no new infra) — window and email-vs-IP scoping are starting guesses, not mentor-confirmed.
- **`chat_messages.is_read` — reversed the earlier keep-the-column decision.** Originally kept permanently `0` since chat is a live billed session, not async inbox messaging. Decision reversed this session: dropped via migration, removed from all 4 call sites (`messages.php` GET, POST insert, POST follow-up select; `history.php` transcript select).

### Testing
- Full artifact set for the 4 new endpoints: 46-case test plan (landscape six-column, sectioned), screenshot template, Postman collection (68 requests, 4 folders).
- Postman collection enhanced with ID chaining (`history_session_id`, `enquiry_id`, `earning_id` auto-captured via post-response scripts), PIN centralization (4 identities → 4 collection variables instead of 18 inline literals), dynamic unique test emails for the rate-limit tests, and pre-request guards that console-warn when a chained variable is missing/stale.
- SQL proof queries — 46 total, one per test case, matched 1:1 to test plan numbering; block-comment style throughout.
- BE-10 addendum: 5 new test cases for the `?status=` filter — test plan, screenshot template, and the **first-ever Postman collection for `pending.php`** (Day 6 testing was originally done in Thunder Client).

### Bugs caught during build/review
- **PHP `const` inside conditional blocks — parse error.** `enquiries.php`'s first draft declared `const` five times inside `if`/`elseif` branches; PHP only allows `const` at top-level file scope. Hoisted all five, unconditionally, to the top of the file.
- **`enquiries.php` — logged-in submitter's `user_id` not captured, caught live.** Root cause: the public POST branch never calls `requireAuth()`, which is what implicitly starts/resumes the session on every other endpoint that reads `$_SESSION` successfully. Fixed with an explicit `session_status() === PHP_SESSION_NONE` guard before reading `$_SESSION['user_id']`.
- **`messages.php` left in a broken state mid-`is_read`-removal.** The POST branch's `INSERT` and follow-up `SELECT` still referenced the dropped column after the migration ran; caught and fixed before it caused a live 500 on every message send.

### Open items flagged for mentor
1. BE-17's list-vs-transcript split via `?session_id=`.
2. BE-18b's Option A vs Option B settlement design.
3. BE-19b (resolve) — needs a ticket number for the tracking sheet.
4. BE-19's rate-limit window/threshold and email-vs-IP scoping.
5. Whether `requireAuth()`'s generic "Forbidden" message should be more descriptive — an app-wide middleware change affecting every admin-gated endpoint, not just today's new ones. Deferred pending decision.

### Deferred
- Bulk/batch settlement action for mark-paid (settle-all-unpaid-for-doctor-X).
